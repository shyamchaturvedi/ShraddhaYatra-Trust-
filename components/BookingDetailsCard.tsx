...
const BookingDetailsCard: React.FC<BookingDetailsCardProps> = ({ booking, trip, user }) => {
  const formattedDate = new Date(trip.date).toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  ...
  return (
    <div className="bg-white rounded-lg shadow-2xl max-w-2xl mx-auto my-8 p-6 md:p-8 border-4 border-amber-400 printable-card">
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-6">
        ...
        <div className="flex items-center justify-center">
            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=BookingID:${booking.id}-TripID:${trip.id}`} alt="QR Code" />
        </div>
      </div>
